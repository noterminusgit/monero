...
- ~~**`src/wallet/api/wallet.h:270`** -- TODO: harden password handling in the wallet API. The wallet password is stored as a plain `std::string m_password` member, which may persist in memory. References related discussions in monero-gui #1537, feather #72, monero #8619.~~ **FIXED:** Changed `m_password` from `std::string` to `epee::wipeable_string` so password material is securely wiped from memory on destruction.
